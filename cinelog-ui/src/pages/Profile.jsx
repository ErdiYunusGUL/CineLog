...
function Profile() {
    ...
    // YENİ: YENİ LİSTE OLUÅTURMA FONKSİYONU
    ...
    const handleFileUpload = (e) => {
        ...
        reader.onload = (event) => {
            ...
            if (nameIndex === -1) {
                alert("Bu CSV dosyası uyumlu değil. Lütfen 'Name' veya 'Title' sütunu içeren geçerli bir dosya yükleyin.");
                return;
            }
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                // Virgülden ayır ama tırnak içindeki virgülleri yoksay
                const cols = line.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/);
                
                if (cols.length > nameIndex) {
                    let movieName = cols[nameIndex].trim().replace(/^"|"$/g, ''); // Tırnakları temizle
            ...
            // Inputu sıfırla ki aynı dosyayı tekrar yükleyebilsin
    ...
    return (
        <div className="container mt-4 mb-5 text-light">
            ...
            {/* ALL-TIME STATS VE İÇE/DIÅA AKTAR BUTONLARI */}
            ...
            {/* YENİ: MODAL (YENİ LİSTE OLUÅTURMA) */}
                        ...
                        <div className="modal-content bg-dark text-light border border-secondary shadow-lg glass-card">
                            ...
                            <div className="modal-body">
                                <p className="text-muted small mb-3">
                                    Yüklediğiniz dosyadan <strong>{importPreview.length}</strong> adet film tespit edildi. Bu filmler izleme geçmişinize eklenecektir.
                                </p>
                            ...
                            <div className="modal-footer border-top border-secondary">
                                <button type="button" className="btn btn-outline-secondary" onClick={() => setShowImportModal(false)}>İptal</button>
                                <button type="button" className="btn btn-danger fw-bold px-4" onClick={confirmImport} disabled={importLoading}>
                                    {importLoading ? 'Ekleniyor...' : 'Tümünü Ekle'}
                                </button>
                            </div>
                        </div>
                    </div>
